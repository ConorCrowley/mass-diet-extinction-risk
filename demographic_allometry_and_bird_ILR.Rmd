...
# Define a discrete viridis colour palette for Diet_5Cat with fixed ordering
# (uses viridisLite, which ggplot2 already depends on)
diet_pal <- setNames(
  viridisLite::viridis(length(diet_levels)),
  diet_levels
)
...
# ---- NEW: add (a)/(b) labels directly to the panel plots ----------------

p_diet_comp_lab <- ggdraw() +
  draw_plot(p_diet_comp_nt) +
  draw_plot_label(
    label    = "(a)",
    x        = 0,
    y        = 1,
...
p_diet_scatter_lab <- ggdraw() +
  draw_plot(p_diet_scatter_nt) +
  draw_plot_label(
    label    = "(b)",
    x        = 0,
    y        = 1,
...
# Build left and right columns (title + *labelled* panel)
left_block  <- plot_grid(title_left,  p_diet_comp_lab,    ncol = 1, rel_heights = c(0.12, 1))
right_block <- plot_grid(title_right, p_diet_scatter_lab, ncol = 1, rel_heights = c(0.12, 1))
...
# Place panels side by side, add (a)/(b) tags, and collect the legend at the bottom
fig_ilr_compare <- (p_ilr_bin + p_ilr_qua +
...
p_ilr_tern_diagram <- ggtern(
  ...
  # Continuous palette for predicted σ can stay viridis/plasma
...
# Remove legends from the main panel objects and add (a)/(b) tags
p_ilr_tern_scatter_nl <- p_ilr_tern_scatter +
  guides(color = "none") +
  labs(tag = "(a)") +
  theme(
    plot.tag          = element_text(face = "bold", size = 12),
    plot.tag.position = c(0.1, 0.99)  # left panel tag
  )

p_ilr_tern_diagram_nl <- p_ilr_tern_diagram +
  theme(legend.position = "none") +
  labs(tag = "(b)") +
  theme(
    plot.tag          = element_text(face = "bold", size = 12),
    # nudge (b) further into the left margin to avoid overlapping the title
...
fig_ilr_performance <- (panels_row / wrap_elements(leg_diet)) +
  ...
  plot_annotation(
    theme = theme(plot.margin = margin(6, 6, 22, 6))
  )
