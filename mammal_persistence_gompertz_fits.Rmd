...
# Combine the two panels side by side with (a)/(b) labels
fig_gomp_elephant <- (p_ele_mod + p_ele_fixed + 
...
p_curves_left <- ggplot(df_curves_left, aes(K, p, group = idx, color = logM)) +
  ...
  labs(
    title = "Higher starting pop. range",
    x     = "Starting population K",
    y     = "Persistence probability",
...
p_curves_right <- ggplot(df_curves_right, aes(K, p, group = idx, color = logM)) +
  ...
  labs(
    title = "Lower starting pop. range",
    x     = "Starting population K",
    y     = NULL,
    color = expression(log[10](Mass~"(g)"))
  ) +
...
fig_gomp_params_b <- ggplot(df_params, aes(x = logM, y = b)) +
  ...
  labs(
    title = "Gompertz parameter c",
    x     = expression(log[10](Mass~"(g)")),
    y     = "c"
  ) +
  theme_params()

fig_gomp_params <- (fig_gomp_params_a + fig_gomp_params_b +
  plot_layout(widths = c(1, 1))) +
  plot_annotation(
    tag_levels = "a",   # a, b, ...
    tag_prefix = "(",   # -> (a), (b)
