...
---
title: "bird_patches_connectivity"
output: html_document
---

```{r}
###############################################################################
# bird patches + connectivity builder
...
species_df <- read.csv("Data/Clean/bird_data.csv", stringsAsFactors = FALSE)
...
# Presence rasters: files in Spatial/bird_ppm_bin, names → lower + no "_bin"
tif_files <- list.files("Spatial/bird_ppm_bin", pattern = "\\.tif$", full.names = TRUE)
tif_names <- tools::file_path_sans_ext(basename(tif_files))
tif_names <- tolower(sub("_bin$", "", tif_names))

resolved <- tolower(trimws(species_df$scientificName))
...
saveRDS(all_patch_lookup, file = file.path("Spatial", "all_bird_patch_lookup.rds"))
saveRDS(all_connectivity, file = file.path("Spatial", "all_bird_connectivity.rds"))
```