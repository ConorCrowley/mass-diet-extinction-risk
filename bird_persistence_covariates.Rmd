...
birds <- birds %>%
  ...
  # Attach species-level effect (aligned with scientificName)
  left_join(
...
birds <- birds %>%
  mutate(
    diet_combo = sprintf(
      "%d_%d_%d",
      round(Diet.Inv),
      round(Diet.AllPlants),
      round(Diet.VertFishScav)
    ),
...
# Directory where the per-combo LOESS files live.
# In your Gompertz script they were saved in "Data/Clean" as:
#   bird_loess_fit_a_<combo>.rds
#   bird_loess_fit_b_<combo>.rds
loess_dir <- dirname(PATHS$bird_loess_fits)  # "Data/Clean"

# Unique diet combinations that actually occur in birds with valid mass
combo_vals <- sort(unique(birds$diet_combo[is.finite(birds$mass_log10)]))

# For each diet combination, try to load its LOESS models and predict a_pred, b_pred
for (cmb in combo_vals) {
  # Rows of 'birds' with this diet combo and valid log-mass
  idx <- which(birds$diet_combo == cmb & is.finite(birds$mass_log10))
  if (!length(idx)) next

  # Expected filenames for this combo
  file_a <- file.path(loess_dir, paste0("bird_loess_fit_a_", cmb, ".rds"))
  file_b <- file.path(loess_dir, paste0("bird_loess_fit_b_", cmb, ".rds"))

  # Skip if either LOESS file is missing
  if (!file.exists(file_a) || !file.exists(file_b)) {
    # Optional: message to see which combos have no LOESS fit
    # message("No LOESS fits found for combo ", cmb, "; leaving a_pred/b_pred as NA.")
    next
  }

  # Load LOESS models for this diet combo
  loess_a <- readRDS(file_a)
  loess_b <- readRDS(file_b)

  # Predict log10(a) and b at this species' mass, then back-transform a
  newdat  <- data.frame(logM = birds$mass_log10[idx])
  a_vals  <- 10 ^ predict(loess_a, newdata = newdat)
  b_vals  <-      predict(loess_b, newdata = newdat)
